...
use voxelot::{cull_visible_voxels, Camera, World, WorldPos};
...
fn main() {
    ...
    if let Some(voxel) = root.get(0, 0, 0) {
        match voxel {
            ...
            voxelot::Voxel::Chunk(sub) => {
                println!("   - Voxel is Chunk with {} voxels", sub.count())
            }
        }
    }
    ...
    let length = (dir_to_origin[0] * dir_to_origin[0]
        + dir_to_origin[1] * dir_to_origin[1]
        + dir_to_origin[2] * dir_to_origin[2])
        .sqrt();
    let forward = [
    ...
    let camera_close = Camera::new([20.0, 20.0, 20.0], forward, [0.0, 1.0, 0.0]);

    let visible_close = cull_visible_voxels(&world, &camera_close);
    println!(
        "   Close camera (20 units away): {} visible voxels",
        visible_close.len()
    );
    ...
    let length_far =
        (dir_far[0] * dir_far[0] + dir_far[1] * dir_far[1] + dir_far[2] * dir_far[2]).sqrt();
    let forward_far = [
    ...
    let camera_far = Camera::new([300.0, 300.0, 300.0], forward_far, [0.0, 1.0, 0.0]);

    let visible_far = cull_visible_voxels(&world, &camera_far);
    println!(
        "   Far camera (300 units away): {} visible voxels",
        visible_far.len()
    );
    ...
    world
        .subdivide_at(WorldPos::new(100, 0, 0))
        .expect("Failed level 1");
