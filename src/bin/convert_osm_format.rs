...
use std::path::Path;
use voxelot::{load_world_file, save_world_file, World, WorldPos};
...
fn main() -> std::io::Result<()> {
    ...
    println!(
        "Using depth {} (world size: {}³)",
        depth,
        16i64.pow(depth as u32)
    );
    ...
    println!(
        "Old size: {} bytes ({:.1} MB)",
        old_size,
        old_size as f64 / 1_000_000.0
    );
    println!(
        "New size: {} bytes ({:.1} KB)",
        new_size,
        new_size as f64 / 1_000.0
    );
    ...
    println!(
        "Successfully loaded world with depth {}",
        loaded_world.hierarchy_depth()
    );
    ...
    if loaded_world.count() == world.count() {
        ...
        println!(
            "✗ WARNING: Voxel count mismatch! Original: {}, Loaded: {}",
            world.count(),
            loaded_world.count()
        );
    }
