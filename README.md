...
A few years ago, while working on a database engine ([Positorium](https://github.com/roenbaeck/positorium)), I first encountered Roaring Bitmaps. Ever since I was a child, I’d dreamed of making a voxel-based game. The idea of building an entire engine around bitmaps—using bitwise operations for as much of the logic as possible—suddenly felt practical thanks to Roaring Bitmaps. Both culling and meshing are traditionally CPU-heavy, but Roaring makes them extremely fast. From a memory standpoint, the approach is also remarkably efficient, keeping RAM usage very low.

For the rendering and shading side, I studied a wide range of modern techniques. Screen-space indirect lighting and occlusion have seen impressive progress in recent years. One particularly relevant technique is SSILVB (Screen-Space Indirect Lighting with Visibility Bitmask), described in this excellent [article by cybereality](https://cybereality.com/screen-space-indirect-lighting-with-visibility-bitmask-improvement-to-gtao-ssao-real-time-ambient-occlusion-algorithm-glsl-shader-implementation/). Because it also relies heavily on bitmasks, it served as a direct inspiration.

Depth-of-field is fairly standard, but I quickly discovered that a traditional Gaussian blur produces severe artifacts on voxel geometry almost regardless of kernel size. FrostKiwi’s in-depth [article on Dual Kawase Blur](https://blog.frost.kiwi/dual-kawase/) solved the problem perfectly, so I adopted that method. The same Kawase passes are reused for bloom, which turned out to be straightforward to implement.

Water shading borrows many ideas from Alexander Ameye’s beautiful stylized [water shader tutorial](https://ameye.dev/notes/stylized-water-shader/). The sky is simply a slowly rotating cubemap texture. Shadows come from a classic shadow map tied to a single directional light that acts as the sun.

In short, the project is a love letter to bit-level optimization on the CPU side (thanks to Roaring Bitmaps) combined with modern, carefully chosen screen-space and post-processing techniques on the GPU.
